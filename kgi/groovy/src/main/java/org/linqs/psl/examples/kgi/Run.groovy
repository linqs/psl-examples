	...
	private void loadData() {
		...
		for (String type : ["learn", "eval"]) {
			...
			for (StandardPredicate predicate : dataStore.getRegisteredPredicates()) {
				...
				InserterUtils.loadDelimitedDataTruth(inserter, path);
			}

			Inserter inserter = dataStore.getInserter(Cat, targetsPartition);
			InserterUtils.loadDelimitedData(inserter, Paths.get(DATA_PATH, type, "CAT_targets.txt").toString());

			inserter = dataStore.getInserter(Rel, targetsPartition);
			InserterUtils.loadDelimitedData(inserter, Paths.get(DATA_PATH, type, "REL_targets.txt").toString());

			inserter = dataStore.getInserter(Cat, truthPartition);
			InserterUtils.loadDelimitedDataTruth(inserter, Paths.get(DATA_PATH, type, "CAT_truth.txt").toString());

			inserter = dataStore.getInserter(Rel, truthPartition);
			InserterUtils.loadDelimitedDataTruth(inserter, Paths.get(DATA_PATH, type, "REL_truth.txt").toString());
		}
	}
