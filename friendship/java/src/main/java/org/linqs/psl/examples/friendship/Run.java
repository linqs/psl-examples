...
public class Run {
    ...
    private void definePredicates() {
        model.addPredicate("Similar", ConstantType.UniqueIntID, ConstantType.UniqueIntID);
        model.addPredicate("Block", ConstantType.UniqueIntID, ConstantType.UniqueIntID);
        model.addPredicate("Friends", ConstantType.UniqueIntID, ConstantType.UniqueIntID);
    }
    ...
    private void defineRules() {
        ...
        model.addRule("10: Block(P1, A) & Block(P2, A) & Similar(P1, P2) & P1 != P2 -> Friends(P1, P2) ^2");

        model.addRule("10: Block(P1, A) & Block(P2, A) & Block(P3, A) & Friends(P1, P2) & Friends(P2, P3) & P1 != P2 & P2 != P3 & P1 != P3 -> Friends(P1, P3) ^2");

        model.addRule("10: Block(P1, A) & Block(P2, A) & Friends(P1, P2) & P1 != P2 -> Friends(P2, P1) ^2");

        model.addRule("1: !Friends(P1, P2) ^2");
    ...
    private void loadData(Partition obsPartition, Partition targetsPartition, Partition truthPartition) {
        ...
        Inserter inserter = dataStore.getInserter(model.getStandardPredicate("Block"), obsPartition);
        inserter.loadDelimitedData(Paths.get(DATA_PATH, "location_obs.txt").toString());

        inserter = dataStore.getInserter(model.getStandardPredicate("Similar"), obsPartition);
        inserter.loadDelimitedDataTruth(Paths.get(DATA_PATH, "similar_obs.txt").toString());

        inserter = dataStore.getInserter(model.getStandardPredicate("Friends"), targetsPartition);
        inserter.loadDelimitedDataTruth(Paths.get(DATA_PATH, "friends_targets.txt").toString());

        inserter = dataStore.getInserter(model.getStandardPredicate("Friends"), truthPartition);
    ...
    private void runInference(Partition obsPartition, Partition targetsPartition) {
        ...
        StandardPredicate[] closedPredicates = new StandardPredicate[]{model.getStandardPredicate("Friends")};
    ...
    private void writeOutput(Partition targetsPartition) throws IOException{
        ...
        for (GroundAtom atom : resultsDB.getAllGroundAtoms(model.getStandardPredicate("Friends"))) {
    ...
    private void evalResults(Partition targetsPartition, Partition truthPartition) {
        ...
        Database truthDB = dataStore.getDatabase(truthPartition, new StandardPredicate[]{model.getStandardPredicate("Friends")});

        Evaluator eval = new DiscreteEvaluator();
        eval.compute(resultsDB, truthDB,model.getStandardPredicate("Friends"));
